...
@Component({
  selector: 'app-chacra-plexo-solar',
  templateUrl: './plexo-solar.page.html',
  styleUrls: ['./plexo-solar.page.scss'],
  standalone: true,
...
export class PlexoSolarPage {
  ...
  private async carregarIndice(): Promise<void> {
    try {
      const data = await fetch('assets/data/doencas/indice_plexo.json').then((r) => r.json());

      this.indicePlexo = data;
